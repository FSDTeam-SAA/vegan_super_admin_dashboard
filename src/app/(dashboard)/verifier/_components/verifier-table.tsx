...
import { VerifiersUser, VerifiersUserResponse } from "@/types/verifiers";
...
import { verifiersColumns } from "./verifiers-column";
...
const VerifiersTable = () => {
  ...
  const {
    ...
    isError,
    error,
  } = useQuery<VerifiersUserResponse>({
    queryKey: ["merchants", country, state, city, currentPage],
    queryFn: () =>
      fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/v1/superadmin/getAllVerifiers?country=${country}&state=${state}&city=${city}&page=${currentPage}&limit=10`
      ).then((res) => res.json()),
  });
  ...
  if (isLoading) {
    ...
    content = (
      <div>
        ...
        <TableContainer data={response.data ?? []} columns={verifiersColumns} />
...
interface Props {
  data: VerifiersUser[]; // Replace 'any' with the actual type of your data
  columns: ColumnDef<VerifiersUser>[]; // Replace 'any' with the actual type of your columns
}
